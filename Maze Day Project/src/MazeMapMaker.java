...
public class MazeMapMaker extends JFrame {
	...
	public MazeMapMaker() {
		...
		if (level != -1) {
			...
			this.setTitle("Maze Map Creator");
			this.setLayout(null);
	...
	public void getMapList() {
		for (int i = 0; i < 99; i++ ) {
			...
			if (map.exists()) {
				System.out.println("Level "+i+" exists already.");
	...
	public void getLevelChoice() {
		if (levelsExistAlready) {
			...
			String choice = (String) JOptionPane.showInputDialog(null,
					"Which level would you like to play?", "Maze Level Selector",
	...
	public void loadMap() {
		try {
			...
			System.out.println("Unable to load existing map, creating a new map.");
			for (int y = 0; y < columns; y++) {
				for (int x = 0; x < rows; x++) {
					map[x][y] = 0;
				}
